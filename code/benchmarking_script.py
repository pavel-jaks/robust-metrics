...
def main():
    ...
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_l2=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_dssim5=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_dssim10=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_dssim20=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_dssim100=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_l1=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_l0=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_linf=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
    for i, pair in enumerate(image_pairs):
        benchmark(f'{metric_wasserstein=}'.split('=')[0], pair, 10, f"test_images{i + 1}")
