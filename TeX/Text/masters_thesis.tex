...
dále od tohoto odvozené metody iterativního charakteru
...
tzv. \emph{obraná destilace} (z angl. \emph{defensive distillation}; \cite{distillation}, 2016),
...
Pátá kapitola přináší pororvnání naimplementovaných metrik mezi sebou, co se výpočetní náročnosti týče.
...
Nakonec uveďme, že součástí této práce je i příloha která obsahuje matematické důkazy k použitým tvrzením
...
lze nahlédnout v kapitole Příloha: Důkazy.
...
a následně agregován, je u názvu této metriky uvedena i velikost tohoto podokna, a to pokud je menší než velikost rozměry obrázku.
...
\chapter{Příloha: Důkazy}
% \addcontentsline{toc}{chapter}{Příloha: Důkazy}

\section{Metrické vlastnosti Wassersteinovy vzdálenosti}

\emph{Tvrzení:} Mějme metrický prostor $(X, \rho)$, dále pak mějme $p \in [1, +\infty)$.
...
\begin{align}
	\mathbb{E}_{(x, y) \sim \tilde{\gamma}} \rho (x, y)^p &= \int_X \int_X \rho (x, y)^p \cdot 1_{x = y} \cdot \mu(x) d x d y, \\
	&= 0.
\end{align}
...
Potom z druhého z předpokladů plyne, že existuje $r \in X$ takové,
že $\mu (r) \neq \nu (r)$, bez újmy na obecnosti předpokládejme, že $\mu (r) > \nu (r)$,
a že existuje jiné $s \in X$ takové,
že $\mu(s) < \nu (s)$,
poněvadž integrál obou měr musí být roven $1$.
Pak ale pro každé $\gamma \in \Gamma (\mu, \nu)$ platí, že
\begin{equation}
	\mathbb{E}_{(x, y) \sim \gamma} \rho (x, y)^p = \int_X \int_X \rho(x, y)^p \gamma(x, y) dx dy \geq \delta,
\end{equation}
pro nějaké $\delta > 0$,
...
existuje jistě takové $\gamma_1 \in \Gamma (\mu, \nu)$, že
\begin{equation}
	\int_X \int_X \rho (x, y)^p \gamma_1 (x, y) dx dy < W_p (\mu, \nu)^p + \varepsilon \label{inf1}
\end{equation}
pro libovoně malé $\varepsilon > 0$.
...
Pak platí:
\begin{eqnarray}
	\left[\int_X \int_X \rho (x, z)^p \gamma_3 (x, z) dx dz\right]^\frac{1}{p} &\leq& \left[\int_X \int_X \left[ \rho(x,y) + \rho(y,z) \right]^p \gamma_3 (x, z) dx dz \right]^\frac{1}{p} \label{metric} \\
	&\leq& \left[\int_X \int_X \int_X \frac{\rho (x, y)^p \cdot \gamma_1 (x, y) \cdot \gamma_2 (y,z)}{\nu (y)} dz dx dy \right]^\frac{1}{p}  \label{Minkowski} \\
	& & + \left[\int_X \int_X \int_X \frac{\rho (y, z)^p \cdot \gamma_1 (x, y) \cdot \gamma_2 (y,z)}{\nu (y)} dx dy dz \right]^\frac{1}{p} \label{Minkowski_2} \\
	&\leq& \left[W_p(\mu, \nu)^p + \varepsilon \right]^\frac{1}{p} + \left[ W_p (\nu, \eta)^p + \varepsilon\right]^\frac{1}{p}. \label{After_Mink}
\end{eqnarray}
Nerovnost v (\ref{metric}) vychází z metrických vlastností $\rho$.
Dále jsme při přechodu na řádky (\ref{Minkowski}) a (\ref{Minkowski_2}) využili Minkowského nerovnost.
